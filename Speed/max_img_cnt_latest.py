...
def del_temp(vehicle_id):
    ...
    if os.path.exists(vehicle_folder) and track_id not in reported_violations:
        ...
        if vehicle_id not in current_vehicles:
            if vehicle_id not in vehicle_last_seen:
                ...
                if vehicle_id not in reported_violations:
                    ...
                    transfer_violation_folder("D:/code/Cropper/speed/temp_vehicles", "D:/code/Cropper/speed/violation4", vehicle_id)
        else:
            vehicle_last_seen[vehicle_id] = frame_count
